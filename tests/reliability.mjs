...
try {
  ...
  assert.equal(await frame.locator('.dsa-capture').count(),0)
  await frame.locator('.dsa-pin').click()
  assert.equal(await frame.locator('.dsa-comment-text').textContent(),'保持按钮与说明对齐')
  assert.equal(await frame.locator('.dsa-card textarea').count(),0)
  assert.equal(await marker.getAttribute('aria-pressed'),'false')
  await frame.locator('#draft').click()
  assert.equal(await frame.locator('.dsa-card').count(),0)
  assert.equal(await frame.locator('.dsa-pin').count(),1)
  assert.equal(await frame.locator('.dsa-capture').count(),0)
  await marker.click()
  await frame.locator('.dsa-pin').click()
  assert.equal(await frame.locator('.dsa-comment-text').textContent(),'保持按钮与说明对齐')
  ...
  assert.equal(await frame.locator('.dsa-card').count(),0)
  await marker.click()
  await frame.locator('#draft').click({force:true})
  assert.equal(await frame.locator('.dsa-card textarea').count(),1)
  await frame.locator('#route').click({force:true})
  assert.equal(await frame.locator('.dsa-card').count(),0)
  assert.equal(await marker.getAttribute('aria-pressed'),'false')
  assert.equal(await frame.evaluate(()=>location.pathname),'/')
  pass('saved annotation exits picking; pin views comment; outside clicks close without making a new annotation')
  ...
  await frame.locator('#draft').fill('keep me');await frame.locator('#route').click();await page.waitForTimeout(120)
  ...
  await frame.waitForFunction(()=>{const pin=document.querySelector('.dsa-pin');return pin&&/^View annotation/.test(pin.getAttribute('aria-label')||'')})
